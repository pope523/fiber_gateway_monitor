...
***REMOVED*** Get the current directory (where the script is run from)
Set-Location $PSScriptRoot
$projectRoot = Get-Location

***REMOVED*** Ensure .vscode directory exists
...
***REMOVED*** Path to workspace settings.json
$settingsPath = Join-Path $vscodeDir "settings.json"

***REMOVED*** Read existing settings or create an empty hash table
$settings = @{}
if (Test-Path $settingsPath) {
    $jsonContent = Get-Content $settingsPath | Out-String
    ***REMOVED*** Handle empty or invalid JSON gracefully
...
***REMOVED*** Ensure 'python' and 'python.testing' hash tables exist
...
***REMOVED*** Enable pytest and configure arguments if not already set
...
if ($settings.python.testing.cwd -eq $null) {
    ***REMOVED*** Set the current working directory for tests to the workspace folder
    ***REMOVED*** This is often important for finding modules correctly during test runs
...
if ($settings.python.testing.pytestArgs -eq $null) {
    ***REMOVED*** Configure pytest to look for tests in the 'tests' folder within the workspace root
    $settings.python.testing.pytestArgs = @("tests")
    Write-Host "Configured pytest test arguments."
}

***REMOVED*** Write updated settings back to settings.json
...
***REMOVED*** Optional: Open VS Code after script runs
