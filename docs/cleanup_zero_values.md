***REMOVED*** Cleaning Up Zero Values from Home Assistant History
...
***REMOVED******REMOVED*** Method 1: Using Home Assistant Developer Tools (Recommended)
...
data:
  entity_id:
    ...
    ***REMOVED*** Add all your cable modem sensor entities here
  keep_days: 0  ***REMOVED*** Will purge all history
```

**Note**: This will delete ALL history for the specified entities. If you want to keep good data, use Method 2 instead.

***REMOVED******REMOVED*** Method 2: Direct Database Cleanup (SQLite)
...
***REMOVED******REMOVED******REMOVED*** Backup your database first:

```bash
***REMOVED*** SSH into your Home Assistant
***REMOVED*** Find your database location (usually /config/home-assistant_v2.db)
cp /config/home-assistant_v2.db /config/home-assistant_v2.db.backup
```

***REMOVED******REMOVED******REMOVED*** Connect to the database:

```bash
sqlite3 /config/home-assistant_v2.db
```

***REMOVED******REMOVED******REMOVED*** Example SQL to find zero values around midnight:
...
***REMOVED******REMOVED******REMOVED*** Delete the zero values (be careful!):
...
***REMOVED******REMOVED******REMOVED*** Exit SQLite:

```sql
.quit
```

***REMOVED******REMOVED******REMOVED*** Restart Home Assistant:

After making database changes, restart Home Assistant for the changes to take effect.

***REMOVED******REMOVED*** Method 3: Use the Recorder Integration Settings
...
***REMOVED******REMOVED*** Verifying the Fix
