...
class MotorolaGenericParser(ModemParser):
    ...
    priority = 50  ***REMOVED*** Generic fallback parser, try after model-specific parsers

    ***REMOVED*** New authentication configuration (declarative)
    ***REMOVED*** Motorola modems try both plain and Base64-encoded passwords
        ...
        for attempt, pwd in enumerate(passwords_to_try, 1):
            ...
            ***REMOVED*** Security check: Validate redirect stayed on same host or local network
            ...
            if response.url != login_url:
                ...
                ***REMOVED*** Allow redirects if hostnames match
                if response_parsed.hostname != login_parsed.hostname:
                    ***REMOVED*** For local network devices, allow redirects within private IP ranges
                    ***REMOVED*** This handles cases where modems redirect from IP to hostname or between IPs
                    ***REMOVED*** Both hostnames must be present to validate IP addresses
                    if login_parsed.hostname and response_parsed.hostname:
                        try:
                            ***REMOVED*** Try to parse as IP addresses
                            login_ip = ipaddress.ip_address(login_parsed.hostname)
                            response_ip = ipaddress.ip_address(response_parsed.hostname)

                            ***REMOVED*** If both are private/local IPs, allow the redirect (trusted local network)
                            if login_ip.is_private and response_ip.is_private:
                                ...
                                ***REMOVED*** One or both are public IPs - enforce strict matching
                                _LOGGER.error(
                                    "Motorola: Security violation - redirect to different public host: %s", response.url
                                )
                                return False, None
                        except ValueError:
                            ***REMOVED*** Not IP addresses (hostnames) - enforce strict matching for security
                            _LOGGER.error(
                        ...
                        ***REMOVED*** One or both hostnames are None - reject for security
                        _LOGGER.error(
    ...
    def parse(self, soup: BeautifulSoup, session=None, base_url=None) -> dict:
        ...
        ***REMOVED*** Parse system info first to get uptime
    ...
    def _parse_system_info(self, soup: BeautifulSoup) -> dict:
        ...
        try:
            ...
            ***REMOVED*** Lambda must return bool, not bool | None
