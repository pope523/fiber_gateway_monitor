...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    ***REMOVED*** Initialize health monitor for dual-layer diagnostics
    ***REMOVED*** Create SSL context in executor to avoid blocking I/O in event loop
    ...
    def create_ssl_context():
        ...
        ***REMOVED*** Always disable verification for cable modems (see VERIFY_SSL constant in const.py)
    ...
    async def async_update_data():
        ...
        ***REMOVED*** Run health check first (async, non-blocking)
        ...
        try:
            ...
            ***REMOVED*** Add health monitoring data to coordinator data
            ...
            ***REMOVED*** This prevents all entities showing "unavailable" when modem is just rebooting
    ...
    ***REMOVED*** Use async_config_entry_first_refresh only during initial setup (SETUP_IN_PROGRESS)
    ***REMOVED*** During reload, the entry may be in LOADED state, so use regular refresh
        ...
        ***REMOVED*** Use Home Assistant's official recorder purge service
        def clear_db_history():
            ...
            try:
                ...
                ***REMOVED*** Security: Using parameterized query with ? placeholders (not user input)
                ***REMOVED*** The placeholders string only contains "?" characters, values are passed separately
                placeholders = ",".join("?" * len(cable_modem_entities))
                query = "SELECT metadata_id, entity_id FROM states_meta WHERE entity_id IN (" + placeholders + ")"  ***REMOVED*** nosec B608
                ...
                ***REMOVED*** The placeholders string only contains "?" characters, values are passed separately
                ...
                query = (
                    "DELETE FROM states WHERE metadata_id IN (" + placeholders + ") "  ***REMOVED*** nosec B608
                    "AND last_updated_ts < ?"
                )
                ...
                ***REMOVED*** The placeholders string only contains "?" characters, values are passed separately
                placeholders = ",".join("?" * len(cable_modem_entities))
                stats_query = "SELECT id FROM statistics_meta WHERE statistic_id IN (" + placeholders + ")"  ***REMOVED*** nosec B608
                ...
                ***REMOVED*** Security: Using parameterized queries with ? placeholders (not user input)
                ***REMOVED*** The placeholders string only contains "?" characters, values are passed separately
                stats_deleted = 0
                if stats_metadata_ids:
                    ...
                    query = (
                        "DELETE FROM statistics WHERE metadata_id IN (" + placeholders + ") "  ***REMOVED*** nosec B608
                        "AND start_ts < ?"
                    )
                    ...
                    query_short = (
                        "DELETE FROM statistics_short_term "  ***REMOVED*** nosec B608
